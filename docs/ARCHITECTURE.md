...
| Save states | Full-machine snapshots via serde + postcard (`PsxSystem::save_state`/`load_state`), versioned with a format header and BIOS fingerprint. The frontend-owned assets (`psx_core::Ambient`: BIOS, disc image, memory card, GPU log switch, TTY capture) are deliberately excluded and carried over on load and reset through one path — rolling back the card would corrupt real saves, and the TTY is a host-side log whose readers keep a monotonic cursor. Pending host audio (SPU output, decoded XA) and bring-up counters are excluded too. GUI: F5/F9 and toolbar buttons (slot file next to the memory card image); automation: `psxctl savestate/loadstate <path>`. |
