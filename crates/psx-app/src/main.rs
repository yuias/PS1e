...
fn load_disc(path: &str) -> psx_core::cdrom::Disc {
    ...
    if p.extension().is_some_and(|e| e.eq_ignore_ascii_case("cue")) {
        ...
        parse_cue(&cue, dir).unwrap_or_else(|e| panic!("bad cue sheet '{path}': {e}"))
    } else {
        let data =
            std::fs::read(p).unwrap_or_else(|e| panic!("failed to read disc image '{path}': {e}"));
...
fn parse_cue(cue: &str, dir: &std::path::Path) -> Result<psx_core::cdrom::Disc, String> {
    use psx_core::cdrom::{Disc, RAW_SECTOR, Track};
    struct CueTrack {
        number: u8,
        audio: bool,
        pregap: u32,
...
"#;
    ...
    fn cue_rejects_2048_byte_sector_modes() {
        ...
        let e = parse_cue(cue, std::path::Path::new(".")).err().unwrap();
